...
playWaveform(waveform* wave_to_play)
{
  ...
  uint32_t waveform_pulses = getWaveformPulses(wave_to_play);
  
  if (wave_shape == 'p') {
    playSquareWaveform(waveform_initial_frequency, waveform_final_frequency, waveform_time, waveform_pulses);
  } 
  else if (wave_shape == 's') {
    playSineWaveform(waveform_initial_frequency, waveform_final_frequency, waveform_time, waveform_pulses);
  } 
  else {
