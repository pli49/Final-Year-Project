...
/*
 * Play the selected waveform through the audio out pin. Finds the initial frequency, final frequency, wave time and number of pulses,
 ...
 */
void
playSineWaveform(uint32_t waveform_initial_frequency, uint32_t waveform_final_frequency, uint32_t waveform_time, uint32_t waveform_pulses)
{
  ...
  for (uint32_t i = 0; i < waveform_pulses; ++i) {
    ...
    delay(waveform_time); // Delay for period, interrupts still run
    noInterrupts();
